...
void export_best_individual_as_edge_list(
    ...
    const vector<float>& best_weights,
...
void export_graph_as_edge_list(
    const ctqw::graph::Graph& graph,
    const string& filename
) {
    ofstream file(filename);
...
int main() {
    int n = 100;
    float edge_probability = 0.12f;
    unsigned int seed = 2324;

    auto graph = make_erdos_renyi_graph(n, edge_probability, 1.0f, seed);

    export_graph_as_edge_list(graph, "input_graph.txt");

    cout << "Initial graph exported to input_graph.txt" << endl;
    ...
    const size_t chromosome_size = static_cast<size_t>(graph.edge_count());
    ...
    ga.create_uniform_population(0.0f, 2.0f, seed);

    ga.run_with(127, ctqw_evaluator);

    const auto& best_weights = ga.best_chromosome();

    export_best_individual_as_edge_list(graph, best_weights, "output_graph.txt");

    cout << "Best individual exported to output_graph.txt\n";
